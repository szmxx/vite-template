...
function recursion(
  ...
  callback: (item: RouteRecordRaw) => unknown
) {
  menus.forEach((menu) => {
    callback(menu)
...
function recursionMenu(
  menus: RouteRecordRaw[],
  map: Record<string, RouteRecordRaw>,
  result: RouteRecordRaw[]
) {
  menus.forEach((menu) => {
    const origin = map[menu.name as string]
    if (origin) {
      ...
      if (menu?.children?.length || origin.redirect) {
        const routes = filterRedirectRoutes(
          origin.children as RouteRecordRaw[],
          origin.redirect
        )
        recursionMenu(
          routes.concat((menu.children as RouteRecordRaw[]) || []),
          map,
          item.children
        )
      }
